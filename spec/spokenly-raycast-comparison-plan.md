...
MVP は **Raycast Extension**（`getSelectedText()` + Form + `open` deeplink）とする。**Script Command は採用しない**。選択テキストは clipboard ではなく `getSelectedText()` で取得する。

## 目的

任意アプリで選択したテキストを Original に、ユーザー入力した Replacement を Spokenly の Word Replacements 追加フォームを開き、ユーザーが Spokenly 側で Add をクリックして保存する Raycast Extension の仕様を定義する。
...
1. 送信時に `open` で `spokenly://word-replacements/add?from=…&to=…` を開く（`from` / `to` は URL エンコード）
1. 成功 Toast を表示し、Spokenly 側で Add をクリックして保存するよう案内する
...
Extension は word-replacements/add deeplink で Spokenly の追加フォームを開く。`from` / `to` は URL エンコードし、Add をクリックするまで保存されない。

```
spokenly://word-replacements/add?from=<original>&to=<replacement>
```
...
- `spokenly://word-replacements/add?from=test&to=TEST` が手動で Spokenly の追加フォームを開くか
...
- 選択テキストが Original として Spokenly の追加フォームに事前入力される（`getSelectedText` 経由。clipboard は使用しない）
